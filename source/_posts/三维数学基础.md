...
- 在e'坐标系下的向量a'，通过旋转矩阵，可以变换到e坐标系的向量a，两个向量是同一个向量
...
- R的正交化：1、$RR^T=I$，$R^{-1}=R^T$；2、$det(R)=1$
...
- 逆变换矩阵：$T^{-1}=\begin{bmatrix}   R^T &-R^Tt\\0^T&1\end{bmatrix}$
...
# 旋转向量和欧拉角
- 前面用旋转矩阵表示3自由度的刚体旋转，3x3的矩阵包含9个变量，具有冗余的信息。
...
- 三维空间的单位向量$n=[n_x,n_y,n_z]^T$，某个旋转是绕单位向量n进行了角度为θ的旋转，则该旋转四元数的表达形式为：$q=[cos\frac{\theta}{2},n_xsin\frac{\theta}{2},n_ysin\frac{\theta}{2},n_zsin\frac{\theta}{2}]^T=[cos\frac{\theta}{2},\vec{n}sin\frac{\theta}{2}]^T$
...
### 李群到李代数[^1]
[^1]: V.S. Varadarajan, Lie groups, Lie algebars, and their representations, vol. 102. Springer Science & Business Media, 2013.
...
- 变换矩阵李代数se(3)
  ...
  - se(3)的李括号：$[\xi_1,\xi_2]=(\xi_1^\wedge\xi_2^\wedge-\xi_2^\wedge\xi_1^\wedge)^\vee$，这里$\wedge$是将6维向量转为4维矩阵，与so(3)的不同，不代表反对称了：$\xi^\wedge=\begin{bmatrix}   \phi^\wedge &\rho\\0^T&0\end{bmatrix}$
...
- 最后这个形式和罗德里格斯公式形式一致，所以说明李代数实际的物理意义就是旋转向量。
...
### BCH公式
- Baker-Campbell-Hausdorff formula解释了两个李群的乘法再对数之后，得到李代数的加法展开的形式：
  - $ln(exp(A)exp(B))=A+B+\frac{1}{2}[A,B]-\frac{1}{12}[B,[A,B]]+...$
